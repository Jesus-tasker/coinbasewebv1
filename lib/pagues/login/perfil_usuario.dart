...
import 'package:criptodadosweb/providers/login/provider_perfilusuario.dart';
import 'package:criptodadosweb/providers/productos_provider.dart';
...
class _Perfil_UsuarioState extends State<Perfil_Usuario> {
  ...
  final _pref_user = PreferenciasUsuario(); //shared preferences
...
//imagen
  ...
  Widget build(BuildContext context) {
    _coprobarpreferenciasweb(context); //especial para web
    //para app pero aprece que infuye tambien
    //funciona en app pero parece que no en web
    /* if (_pref_user.token != "") {
      //keyUID
      // print(pref.token);
...
/*
  mostrar_fulldialog_mapas(BuildContext context) {
      ...
      child: RaisedButton(
        ...
        onPressed: () async {
          ...
          if (user.usuarioNombre == "" ||
              user.teleonoUsuario == "" ||
              user.direccion == "") {
            mostraralerta(context, "completar todos los campos");
          }
          user.latitud = 1;
          user.longitud = 2;
          /* if (_pref_user.latitud == 0 ) {
            mostraralerta(context, "porfavor obten la ubicacion actual");
          }*/
...
//
            _pref_user.latitud = 1;
            _pref_user.longitud = 2;
            ...
            if (_pref_user.latitud != null || _pref_user.latitud != 0.0) {
              ...
              mostraralerta(context, "indica la ubicacion");
            }
          } else {
...
//3.1 guardar_v2
  ...
  _coprobarpreferenciasweb(BuildContext context) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.getBool('auth');
    user.usuarioNombre = prefs.getString("nombreweb")!;
    user.fotodePerfilUri = prefs.getString("fotoweb")!;
    user.keyUsuario = prefs.getString("tokenweb")!;
    //  user.teleonoUsuario = prefs.getString("telefonoweb")!;
    print("------------webpreferences");
    print(user.keyUsuario);
    print(user.usuarioNombre);
    print(user.fotodePerfilUri);
    print(user.teleonoUsuario); //null
    print("------------webpreferences");
    if (user.usuarioNombre != "") {
      _nombre = user.usuarioNombre;
      _foto = user.fotodePerfilUri;
      _keyuid = user.keyUsuario;
      _keyuid_preference = user.keyUsuario;

      setState(() {});
    }

    // _tel_temp = user.teleonoUsuario;
