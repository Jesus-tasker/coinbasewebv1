...
class RegistroPague extends StatelessWidget {
  final usuaruio_prov = new Ususario_Provider();
  ...
  Widget _loginForm(BuildContext context) {
    final bloc = Provider_bloc.of(context);
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        return RaisedButton(
            ...
            onPressed:
                snapshot.hasData ? () => _register_user(bloc, context) : null);
      },
    );
  }

  _register_user(LoginBloc bloc, BuildContext context) async {
    final info = await usuaruio_prov.nuevousuario(bloc.email, bloc.password);
    if (info['ok']) {
